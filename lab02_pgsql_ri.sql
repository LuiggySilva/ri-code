...
select title from filmes where description::text LIKE '%she%';
...
select title from filmes where description::text ILIKE '%she%';
...
### Os resultados não são os mesmos, as duas primeiras consultas são iguais mas a ultima retorna apenas o filme "Dune"
...
### Os resultados para direct e directed são reduzidos para o verbo direct já director se mantém por não ser um verbo


### QUESTÃO 10

select to_tsvector(description) from filmes where id=1;

### Um dicionario onde a chave é uma palavra e o valor a sua frequência na descrição.
...
### A função to_tsquery() passa o atributo recebido para lowercase
